...
Real HTML from both Sources, committed so that a Future PLC redesign presents
as a failing test rather than a guessing game, and so that Extraction can be
repaired with no network and no cloud account. See
...
to; `corpus.ts` records where each page came from.
